var WTGM = {
		...
		//  texture
		tex : new Image(),
		texSrc : "res/img/sprite.png",
		back : new Image(),
		backSrc : "res/img/back.png",
		showFps: 0,
		fps:0,
		lastFrame:0,
		touching:0,
...
//		inAir : false,
	    ...
	    init: function() {
	        ...
	        WTGM.android = WTGM.ua.indexOf('android') > -1 ? true : false;
	        WTGM.ios = ( WTGM.ua.indexOf('iphone') > -1 || WTGM.ua.indexOf('ipad') > -1  ) ? true : false;
			...
			*/
	    	...
	    	/* Preload!! */
	    	preloadResources(WTGM.canvas, function() {
	    ...
	    */
	    ...
	    handleTouch: function(x,y){
	    	...
	    	if (WTGM.touching){    	
	    	  for (var i = 0; i<WTGM.objects.length; i++){
	    		if (WTGM.objects[i].isInside(pos.x,pos.y)){
	    			...
	    			if (WTGM.hit>1){
	    				var message = new wtMessage('+'+Math.round(addScore/100)+'(x'+WTGM.hit+')',pos.x,pos.y,'#118811');
	    			} else {
	    				var message = new wtMessage('+'+Math.round(addScore/100),pos.x,pos.y,'#118811');
	    			}    				
	    		...
	    		if (!WTGM.highscoreBroken && Math.round(WTGM.score/100) > WTGM.highsore){
	    			var message = new wtMessage('Neuer Highscore!',WTGM.touchX,WTGM.touchY-25,'#118811');
	    			WTGM.objects.push(message);
	    	...
	    	if (!WTGM.hit){    			
    			if (WTGM.score-1000 > 0){
    				WTGM.score-=1000;
    				var message = new wtMessage('-10',pos.x,pos.y,'#aa3333');
    	    		WTGM.objects.push(message);
    			}
	    	}
	    	*/
	      ...
	      if (WTGM.touching && !WTGM.paused){  	
	    	if (!WTGM.hit){ 
    			...
    			if (WTGM.score-1000 > 0 && WTGM.mode<100){
    	    		...
    	    		WTGM.objects.push(message);
    	    		if (WTGM.highscoreBroken && Math.round(WTGM.score/100) < WTGM.highsore){
    	    			var message = new wtMessage('Highscore verloren!',WTGM.touchX,WTGM.touchY-25,'#881111');
        	    		WTGM.objects.push(message);
	    ...
	    render: function(srcX, srcY, destX, destY) {
...
//	    				WTGM.ctx.drawImage(WTGM.tile, Math.round(xpos), Math.round(ypos), tileWidth, tileHeight);
	    ...
	    loop: function() {
	    	...
	    	if (WTGM.showFps){	    		
	    		...
	    		var time = new Date().getTime();
	    		var delta = (time - WTGM.lastFrame)/1000;
	    		WTGM.lastFrame = time;
	    		WTGM.fps = 1/delta;
	    	}	
	    },
	  ...
	  //Try to solve
	    solve: function(answer){
	    	WTGM.paused = 1;
	    	if (WTGM.rightAnswer == answer){
	    		WTGM.message=new Array();
	    		WTGM.message['visible']=true;
	    		WTGM.message['time']=new Date().getTime();
	    		WTGM.message['color']='#33aa33';
	    		WTGM.message['text']='Ja, super!';
	    		WTGM.question=WTGM.question.replace('_',WTGM.answers[WTGM.rightAnswer]);
	    		//alert('Ja, Super!');
	    		//WTGM.generateQuestion();
	    		return true;
	    	} else{
	    		...
	    		WTGM.message['visible']=true;
	    		WTGM.message['time']=new Date().getTime();
	    		WTGM.message['color']='#aa3333';
	    		WTGM.message['text']='Leider falsch!';
	    		//alert('Leider falsch!');
		    	//WTGM.setLevel();	
		    	return false;
	    	}
	    ...
	    setLevel: function() {
	    	WTGM.tileMap = new Array();
	    	for (var row = 0; row < g_level1_1.length; row++) { 
	    		WTGM.tileMap[row] = new Array();
	    		for (var col = 0; col <  g_level1_1[row].length; col++) { 	
	    			WTGM.tileMap[row][col] = new wtTile(g_level1_1[row][col]);
	    			//	    			
	    			//WTGM.tileMap[row][col] = new wtTile(g_level1_1[row][col]['img'],g_level1_1[row][col]['verticalOffset'],g_level1_1[row][col]['horizontalOffset']); //new wtTile(g_level1_1[row][col], 0, 0);
	    		}
	    	}
	    	var json_text = JSON.stringify(WTGM.tileMap);
	        WTGM.paused = 0;
	    },
	    */
